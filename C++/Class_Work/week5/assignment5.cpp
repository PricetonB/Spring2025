//Priceton Braswell
//Assignment 5A 
// **CHANGED FUNCTION NAMES TO m_calcBMI and m_weightClass
...
private:
  ...
  void m_calcBMI(int feet, int inches, float pounds) {
    m_bmi = pounds / ((feet * 12 + inches) * (feet * 12 + inches)) * 703;
  }

   void m_weightClass(float bmi, int body_fat_percent, std::string sex) {
     if (bmi > 25) {
...
public:
  Person(std::string name, int height_inches, int height_feet, std::string sex, float pounds, int fat_percent, int id)
    : m_first_name(name), 
      ...
      m_id(id){
    m_calcBMI(m_height_feet, m_height_inches, m_weight_pounds);
    m_weightClass(m_bmi, m_body_fat_percent, m_sex);
  }
...
private:
  ...
  std::string m_weight_class_names[4] = {"o", "p", "n", "u" };  
