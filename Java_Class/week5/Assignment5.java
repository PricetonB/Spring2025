...
class Person {
    ...
    private void calcBMI(int feet, int inches, float pounds) {
        this.bmi = pounds / ((feet * 12 + inches) * (feet * 12 + inches)) * 703;
    }

    private void weightClass(float bmi, int bodyFatPercent, String sex) {
        if (bmi > 25) {
    ...
    public Person(String name, int heightInches, int heightFeet, String sex, float pounds, int fatPercent, int id) {
        ...
        this.id = id;
        calcBMI(this.heightFeet, this.heightInches, this.weightPounds);
        weightClass(this.bmi, this.bodyFatPercent, this.sex);
    }
...
class Group {
    ...
    private String[] weightClassNames = {"o", "p", "n", "u"};
